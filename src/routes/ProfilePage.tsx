...
export const ProfilePage = () => {
  ...
  useEffect(() => {
    if (user)
      window.api.profile.stats(user.id).then((row) => setStats(row ?? stats));
  }, [user]);

  if (!user) return null;

  const saveProfile = async () => {
    await window.api.db.users.updateSettings(user.id, {
      name,
      businessType,
  ...
  const changePassword = async () => {
    ...
    if (!row || !(await compare(currentPassword, row.password_hash)))
      return setPasswordError("Current password is incorrect");
    await window.api.db.users.updatePassword(
      user.id,
  ...
  const deleteEverything = async () => {
    await window.api.db.run("DELETE FROM products WHERE user_id = ?", [
      user.id,
    ]);
    ...
    <div className="profile-page">
      <Card className="profile-card-large">
        ...
        <div className="profile-stats">
          ...
          <span>
            Total Sales<strong>{stats.total_sales}</strong>
          </span>
          <span>
            Total Revenue
            <strong>{formatCurrency(stats.total_revenue, currency)}</strong>
          </span>
        </div>
        {confirmLogout ? (
      ...
      <div className="profile-forms">
        <Card className="settings-section">
          ...
          <div className="business-grid">
            {businessTypes.map(([value, label, Icon]) => (
              <button
                key={value}
                className={`business-card ${businessType === value ? "business-card-selected" : ""}`}
          ...
          <p className="muted-text">
            Changing this only affects new products created going forward
          </p>
        ...
        <Card className="danger-zone">
          ...
          <p>Permanently removes all products, stock, and sales history</p>
          {deleteStep === 0 ? (
            ...
            <>
              <p>
                This will delete all data for {user.name}. Type your name to
                confirm.
              </p>
              <Input
                value={deleteConfirm}
